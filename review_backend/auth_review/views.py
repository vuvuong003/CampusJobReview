...
# This class is responsible for handling essential functionality for user registration and authentication. 

# allows for interaction with user objects
User = get_user_model()

# This class manages user registration and ensures that a username is unique before creating a new user. 
class RegisterView(APIView):
    ...
    def post(self, request, format=None):
        # checks if username provided already exists in the database then return with an 400 BAD REQUEST
        user = User.objects.filter(username=request.data["username"])
        if len(user) > 0:
            return Response({"data" : {"val" : False, "detail" : "Username Exists"}}, status=status.HTTP_400_BAD_REQUEST)
        # if username is unique, the serializer is instantiated with the request data. 
        serializer = RegisterSerializer(data=request.data)
        # serializer checks if the provided data is valid 
        if serializer.is_valid():
            serializer.save()
            # if valid a new user is created with a success response. If not, then return a BAD REQUEST
...
# This class handles user authenticaion and generates JWTs, providing additional custom data in the response
