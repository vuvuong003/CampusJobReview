...
describe('AddReview Component', () => {
  ...
  test('renders all form fields', () => {
    ...
    expect(screen.getByRole('textbox', { name: /hourly pay/i })).toBeInTheDocument();
    expect(screen.getByRole('textbox', { name: /benefits/i })).toBeInTheDocument();
  ...
  test('updates form state when input values change', () => {
    ...
    fireEvent.change(jobTitleInput, { target: { value: 'Software Engineer', name: 'job_title' } });
    expect(jobTitleInput.value).toBe('Software Engineer');

    const departmentInput = screen.getByRole('textbox', { name: /department/i });
    fireEvent.change(departmentInput, { target: { value: 'Engineering', name: 'department' } });
  ...
  test('submits form successfully', async () => {
    ...
    const recommendationRadio = screen.getByRole('radio', { name: /recommendation 8/i });
  ...
  test('handles form submission error', async () => {
    ...
    await waitFor(() => {
      expect(mockAlert).toHaveBeenCalledWith('There was an error submitting your review');
    });
  });
  ...
  test('renders and selects rating radio buttons', () => {
    ...
    [1, 2, 3, 4, 5].forEach(rating => {
      const radio = screen.getByRole('radio', { name: new RegExp(`rating ${rating}`, 'i') });
