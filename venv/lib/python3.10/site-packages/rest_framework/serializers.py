...
class Serializer(BaseSerializer, metaclass=SerializerMetaclass):
    ...
    def _read_only_defaults(self):
        fields = [field for field in self.fields.values() if (field.read_only) and (
            field.default != empty) and (field.source != '*') and ('.' not in field.source)]
    ...
    def errors(self):
        ...
        if isinstance(
                ret,
                list) and len(ret) == 1 and getattr(
                ret[0],
                'code',
                None) == 'null':
...
class ModelSerializer(Serializer):
    ...
    * Default `.create()` and `.update()` implementations are provided.
    ...
    def get_unique_together_constraints(self, model):
        ...
        for parent_class in [model] + list(model._meta.parents):
            ...
            for constraint in parent_class._meta.constraints:
                if isinstance(
                        constraint,
                        models.UniqueConstraint) and len(
                        constraint.fields) > 1:
                    yield (
                        constraint.fields,
    ...
    def get_unique_together_validators(self):
        ...
        field_sources.update({field.field_name: field.source for field in self.fields.values() if (
            field.read_only) and (field.default != empty) and (field.source != '*') and ('.' not in field.source)})
