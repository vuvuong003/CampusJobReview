        ...
        kwargs = {
            key: (
                copy.deepcopy(
                    ...
                    key not in (
                        'validators',
                        'regex')) else value) for key,
            value in self._kwargs.items()}
...
class ChoiceField(Field):
    ...
    def _set_choices(self, choices):
        ...
        self.choice_strings_to_values = {str(key.value) if isinstance(key, Enum) and str(
            key) != str(key.value) else str(key): key for key in self.choices}
...
class MultipleChoiceField(ChoiceField):
    ...
    def to_representation(self, value):
        return {
            self.choice_strings_to_values.get(
                str(item),
...
class ModelField(Field):
    ...
    """
    default_error_messages = {'max_length': _(
