        ...
        for field in self.model_container._meta._get_fields(reverse=False):
            if isinstance(field, (AutoField,
                ...
                raise ValidationError(
                    f'Field "{field}" of model container:"{
                        self.model_container}" ' f'cannot be of type "{
                        type(field)}"')

            if field.attname != field.column:
                raise ValidationError(
                    f'Field "{field}"  of model container:"{
                        self.model_container}" ' f'cannot be named as "{
                        field.attname}", different from ' f'column name "{
                        field.column}"')
...
class ArrayReferenceManagerMixin:
    ...
    def update_or_create(self, **kwargs):
        ...
        obj, created = super(
            ArrayReferenceManagerMixin, self.db_manager(db)).update_or_create(
            **kwargs)
    ...
    def get_or_create(self, **kwargs):
        db = router.db_for_write(
            ...
            instance=self.instance)
        obj, created = super(
            ArrayReferenceManagerMixin, self.db_manager(db)).get_or_create(
            **kwargs)
        # We only need to add() if created because if we got an object back
        # from get() then the relationship already exists.
        if created:
