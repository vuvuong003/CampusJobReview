...
@dataclass
class TokenAlias:
    alias2token: Dict[str, U[AliasableToken, SQLFunc,
...
class UpdateQuery(DMLQuery):
    ...
    def execute(self):
        ...
        logger.debug(
            f'update_many: {
                self.result.modified_count}, matched: {
                self.result.matched_count}')
...
class InsertQuery(DMLQuery):
    ...
    def _columns(self, statement: SQLStatement):
        tok = statement.next()
        self._cols = [
            token.column for token in SQLToken.tokens2sql(
                tok[1], self)]
...
class Query:
    ...
    def parse(self):
        ...
        try:
            ...
            raise SQLDecodeError(
                f'{sm_type} command not implemented for SQL {
                    self._sql}')
    ...
    def _alter(self, sm):
        try:
            ...
            logger.warning(
                'Not implemented alter command for SQL {}'.format(
                    self._sql))
            raise
        else:
            query.execute()
