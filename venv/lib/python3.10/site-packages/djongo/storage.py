...
class GridFSStorage(Storage):
        ...
        try:
            gridfs.delete(
                gridfs.get_last_version(
                    filename=filename).__getattribute__('_id'))
        except NoFile:
            pass
    ...
    def listdir(self, path):
        ...
        gridfs, filename = self._get_gridfs(path)
        assert not filename
        subcollections = _get_subcollections(
            gridfs.__getattribute__('__collection'))
    ...
    def url(self, name):
        ...
        try:
            file_oid = gridfs.get_last_version(
                filename=filename).__getattr__('_id')
        except NoFile:
